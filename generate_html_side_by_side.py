...
def generate_html(max_items=10):
    ...
    for i in range(min(len(test_data), len(model_results))):
        ...
        if 'round1' in model_item:
            ...
            for ans in model_item['round1'].get('answers', []):
                html_content += f'<div class="answer-item">{ans}</div>'
            
            # Explanations
            html_content += "<strong>Explanations:</strong><br>"
            for exp in model_item['round1'].get('explanations', []):
                html_content += f'<div class="explanation">{exp}</div>'
            
            # Aggregation
            if 'aggregation' in model_item['round1']:
                html_content += f"""
                            <div class="final">
                                <strong>Round 1 Aggregation:</strong> {model_item['round1']['aggregation']}
                            </div>
                """
        ...
        if 'round2' in model_item:
            ...
            # Answers
            html_content += "<strong>Answers:</strong><br>"
            for ans in model_item['round2'].get('answers', []):
                html_content += f'<div class="answer-item">{ans}</div>'
            
            # Explanations
            html_content += "<strong>Explanations:</strong><br>"
            for exp in model_item['round2'].get('explanations', []):
                html_content += f'<div class="explanation">{exp}</div>'
            
            # Aggregation
            if 'aggregation' in model_item['round2']:
                html_content += f"""
                            <div class="final">
                                <strong>Round 2 Aggregation:</strong> {model_item['round2']['aggregation']}
                            </div>
                """
        ...
        if 'round3' in model_item:
            ...
            # Answers
            html_content += "<strong>Answers:</strong><br>"
            for ans in model_item['round3'].get('answers', []):
                html_content += f'<div class="answer-item">{ans}</div>'
            
            # Explanations
            html_content += "<strong>Explanations:</strong><br>"
            for exp in model_item['round3'].get('explanations', []):
                html_content += f'<div class="explanation">{exp}</div>'
            
            # Aggregation
            if 'aggregation' in model_item['round3']:
                html_content += f"""
                            <div class="final">
                                <strong>Round 3 Aggregation:</strong> {model_item['round3']['aggregation']}
                            </div>
                """
...
with open('ramdocs_viewer_side_by_side.html', 'w', encoding='utf-8') as f:
    f.write(html_content)

print(f"HTML文件已生成: ramdocs_viewer_side_by_side.html")